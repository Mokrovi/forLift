...
class WebApp:
    ...
    def _setup_routes(self):
        ...
        @self.app.route('/api/android/mic-volume', methods=['POST'])
        def set_android_mic_volume():
            """API для управления громкостью микрофона на Android"""
            data = request.get_json() or {}
            ip = data.get('ip', '')
            volume = data.get('volume', 1.0)
            ...
            try:
                ...
                if response.status_code == 200:
                    return jsonify({
                        "success": True,
                        "message": f"🔊 Громкость микрофона: {int(volume * 100)}%",
                        "ip": ip
                    })
                else:
                    return jsonify({
